...
import { create_nested_vfs, local_mime_type } from '../lib';
import { resolve } from 'path';

describe('base tests', () => {
    it('Gets Proper MIME Type for HTML', () => {
        const mime_html = local_mime_type(resolve(process.cwd(), '__fixtures/index.html'));
        return expect(mime_html).toMatch(/html/g);
    });
    it('Gets 2 vfs files in a dir of 2 files with a subdir', async () => {
        const vfs_test = await create_nested_vfs('/', resolve(process.cwd(), '__fixtures__'));
